...
double pre_left_front_deg,pre_left_rear_deg,pre_right_front_deg,pre_right_rear_deg;
...
void messageCallback(const geometry_msgs::Twist::ConstPtr& msg){
	...
	float linear_vel,vel_x,vel_y,angular_vel, w,rsin,rcos;
	...
	if((!((msg->linear.y ==0)&&(msg->linear.x ==0)))&&(angular_vel != 0))//回転並行があるとき
	{
...
//１８０度以上回転を小さくする、０度の移り変わり
	...
	else if((msg->linear.y ==0)&&(msg->linear.x ==0)&&(angular_vel != 0))//旋回
	{
		...
		right_front.st_target_deg =0;
			...
			left_front.wh_target_vel = -v;
			left_rear.wh_target_vel = -v;
			right_rear.wh_target_vel = -v;
			right_front.wh_target_vel = -v;
	...
	else if((!((msg->linear.y ==0)&&(msg->linear.x ==0)))&&angular_vel==0)//回転がないとき並行のみ
	{
	...
	pre_left_front_deg = left_front.st_target_deg;
	pre_left_rear_deg = left_rear.st_target_deg;
	pre_right_rear_deg = right_rear.st_target_deg;
	pre_right_front_deg = right_front.st_target_deg;	
