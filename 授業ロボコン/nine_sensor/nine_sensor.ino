...
volatile float accX_offset = -0.0274+0.09;//
...
volatile float magX_offset = -17;//
volatile float magY_offset = 2;//
volatile float magZ_offset;//

volatile float gyroX_offset = -0.406;//
...
void MPU9250() {
  ...
  accX = ax * accRange / 32768.0 + accX_offset;//[G]に変換
  accY = ay * accRange / 32768.0 + accY_offset;//[G]に変換
  accZ = az * accRange / 32768.0 + accZ_offset;//[G]に変換

  if(abs(accX)<0.07)accX = 0;
   if(abs(accY)<0.07)accY = 0;
   if(abs(accZ)<0.07)accZ = 0;

//  gyroX = int(gx * gyroRange / 32768.0 + gyroX_offset);//[deg/s]に変換
//  gyroY = int(gy * gyroRange / 32768.0 + gyroY_offset);//[deg/s]に変換
//  gyroZ =int( gz * gyroRange / 32768.0 + gyroZ_offset);//[deg/s]に変換
